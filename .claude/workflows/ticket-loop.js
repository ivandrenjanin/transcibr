...
const IMPL_PROMPT = [
  ...
  '- Write the failing test FIRST (the ticket names its shape), run it with .\\scripts\\test.ps1 -TestName <package.test_name>, watch it fail for the RIGHT reason, then implement until green, then refactor.',
  '- If .claude\\skills\\implement\\SKILL.md exists in the worktree, read it and follow it.',
  '- You are done only when .\\scripts\\build.ps1 AND .\\scripts\\test.ps1 both pass clean from the worktree root.',
  '',
  'DELIVERY:',
...
function reviewPrompt(round, pr, prior, implConcerns) {
  ...
  return [
    ...
    '3. .\\scripts\\build.ps1 and .\\scripts\\test.ps1 both green in YOUR worktree - run them yourself.',
...
function fixPrompt(round, pr, openFindings) {
  return [
    ...
    '3. Re-run .\\scripts\\build.ps1 and .\\scripts\\test.ps1 clean from the worktree root before you finish.',
