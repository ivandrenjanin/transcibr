...
// it lives here because it reuses `refusal_line` (command_line.odin), the same
...
// and its issue #63 addendum for the same judgement re-taken once `child`,
// `audio` and `artifact` also call `refusal_line` directly.
//
...
batch_setup_message :: proc(
	...
	return refusal_line(fmt.aprintf("%q: %s -- %s", subject, says, framing, allocator = allocator))
}
